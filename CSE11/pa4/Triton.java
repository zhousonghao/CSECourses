 ...
 * Date: 2/4/2014 
 * File: Triton.java 
 * Purpose: Triton.java is the class that define for contruction of visible
 * image of Triton and all the methods that declared in the interfaces
 */
import objectdraw.*;
import java.awt.*;
...
/* 
 * Name: Triton
 * Purpose: In Triton class. In this constructor,
 * the program creates a visible image Triton and 2 framed rec that 
 * surround the Triton visible image and provides all the methods that
 * are declared in the interfacesThis is the constructor for the 
 */
public class Triton implements AnimalCard 
{ 
private static String TRITON_SPEAK = "Triton Eye!!!";	
private VisibleImage triton;
private FramedRect  triton_border1, triton_border2; 
...
/* 
 * Name: Triton
 * Purpose: This is the constructor for the Triton class. In this constructor,
 * the program creates a visible image Triton and 2 framed rec that 
 * surround the Triton visible image
 * Parameters: Image pic, Location origin , DrawingCanvas canvas
 * Return: void
 */
public Triton(Image pic, Location origin , DrawingCanvas canvas) {
	
	triton = new VisibleImage (pic , origin.getX(), origin.getY(), canvas);

	triton_border1 = new FramedRect ( origin.getX(), origin.getY(), 
					FRAMED_REC_WIDTH-BORDER_OFFSET,
				    FRAMED_REC_HEIGHT-BORDER_OFFSET, canvas);

	triton_border2 = new FramedRect ( origin.getX()+BORDER_OFFSET, 
...
public void showHighlight( Color color ){
	...
	triton_border1.setColor(color);
	triton_border2.setColor(color);
...
public Color getHighlightColor(){
	...
	return triton_border1.getColor();
